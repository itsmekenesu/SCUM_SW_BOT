...
class AutoConfigBot:
    ...
    def register_with_vps(self):
        # The callback URL uses the port defined by PORT.
        ...
        def handle_command():
            ...
            try:
                if command == "verify":
                    ...
                    # Extract the message and send it in-game.
                    msg = command[4:].strip()
    ...
    def start(self):
        ...
        # Start heartbeat thread to keep the VPS updated.
        def heartbeat_loop():
            while True:
                try:
                    requests.post(
...
# For testing purposes, instantiate the bot.
if __name__ == "__main__":
    bot_api = AutoConfigBot()
    bot_api.start()
    # Do not start Flask here; it will be served externally (e.g., via Waitress)
    while True:
        time.sleep(10)
